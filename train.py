...
def parse_args():
    ...
    parser.add_argument("--data_path", type=str, default="uniformv5_data_demo", help="train data path") 
    parser.add_argument("--metrics", nargs='+', default=['iou', 'dice'], help="metrics")
    parser.add_argument('--device', type=str, default='cuda')
    parser.add_argument("--lr", type=float, default=1e-4, help="learning rate")
    parser.add_argument("--resume", type=str, default=None, help="load resume") 
    parser.add_argument("--model_type", type=str, default="vit_l", help="sam model_type")
    parser.add_argument("--sam_checkpoint", type=str, default="./pretrain_model/sam_vit_l_0b3195.pth", help="sam checkpoint")
    ...
    parser.add_argument("--rgb_eccentric_only", type=str2bool, default=False, help="only train RGB eccentric adapter")
...
def train_one_epoch(args, model, optimizer, train_loader, epoch, criterion):
    ...
    scratch_iter_metrics = [0] * len(args.metrics)
    scratch_count = 0

    if args.rgb_eccentric_only:
        for n, value in model.named_parameters():
            value.requires_grad = "image_encoder.RGBEccentricAdapter" in n
    else:
    ...
    for batch, batched_input in enumerate(train_loader):
            ...
            # for n, value in model.image_encoder.named_parameters():#只有image_encoder被冻结，adapter和其他都是True
            #      #if "image_encoder.blocks.2.Adapter" or "image_encoder.blocks.5.Adatpter" or "image_encoder.blocks.8.Adapter" or  "image_encoder.blocks.11.Adapter" in n:
            #      if "Adapter" in n:
            #          value.requires_grad = True
            #      else:
            #          value.requires_grad = False
            ...
            #----------------------------------------------------------------------------------------------------------------
                ...
                image_embeddings,interm_embeddings = model.image_encoder(batched_input["image"].half())
            #----------------------------------------------------------------------------------------------------------------
...
################================================================================================================
            ...
            if args.rgb_eccentric_only:
                ...
                image_paths = batched_input.get("image_path", [])
                if isinstance(image_paths, str):
                    image_paths = [image_paths]
                if any("/scratch/" in str(path) or "scratch_label" in str(path) for path in image_paths):
                    scratch_iter_metrics = [scratch_iter_metrics[i] + train_batch_metrics[i] for i in range(len(args.metrics))]
                    scratch_count += 1
                continue
...
################================================================================================================
            ...
            # for n, value in model.named_parameters():#没什么改变
            ...
            if args.save_periodic and int(batch+1) % 200 == 0:
                ...
                save_path = os.path.join(f"{args.work_dir}/models", args.run_name, args.save_path)
    ...
    return train_losses, train_iter_metrics, scratch_iter_metrics, scratch_count
...
def main(args):
    ...
    for epoch in range(0, args.epochs):
        ...
        train_losses, train_iter_metrics, scratch_iter_metrics, scratch_count = train_one_epoch(args, model, optimizer, train_loader, epoch, criterion)
        ...
        if scratch_count > 0:
            scratch_iter_metrics = [metric / scratch_count for metric in scratch_iter_metrics]
            scratch_metrics = {f"scratch_{args.metrics[i]}": '{:.4f}'.format(scratch_iter_metrics[i]) for i in range(len(scratch_iter_metrics))}
        else:
            scratch_metrics = {f"scratch_{metric}": 'nan' for metric in args.metrics}

        average_loss = np.mean(train_losses)
        lr = scheduler.get_last_lr()[0] if args.lr_scheduler is not None else args.lr
        loggers.info(f"epoch: {epoch + 1}, lr: {lr}, Train loss: {average_loss:.4f}, metrics: {train_metrics}, scratch_metrics: {scratch_metrics}, scratch_count: {scratch_count}")

        # if average_loss < best_loss or epoch == (args.epochs-1):
        #     best_loss = average_loss
        #     #save_path = os.path.join(args.work_dir, "models", args.run_name, f"epoch{epoch+1}_sam.pth")
        #     save_path = os.path.join(args.work_dir, "models", args.run_name, args.save_path)#=================================================
        ...
        if float(train_metrics['dice'])>best_metrics:
            ...
            save_path = os.path.join(args.work_dir, "models", args.run_name, f'{args.save_path_bmt}_best.pth')#=============================================
            state = {'model': model.float().state_dict(), 'best_metrics': best_metrics, 'epoch': epoch}
            torch.save(state, save_path)
