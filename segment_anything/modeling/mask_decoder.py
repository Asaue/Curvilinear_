...
class MaskDecoder(nn.Module):
    ...
    def predict_masks(
        ...
        # Expand per-image data in batch direction only when there are multiple prompts per image.
        if image_embeddings.shape[0] == tokens.shape[0]:
            src = image_embeddings
            hq_src = hq_features
        else:
            repeat_factor = tokens.shape[0] // image_embeddings.shape[0]
            src = torch.repeat_interleave(image_embeddings, repeat_factor, dim=0)
            hq_src = torch.repeat_interleave(hq_features, repeat_factor, dim=0)
        ...
        upscaled_embedding_hq = self.embedding_maskfeature(upscaled_embedding_sam) + hq_src
