...
def _build_sam(
    ...
    if checkpoint is not None:
        with open(checkpoint, "rb") as f:
            ...
            with safe_globals([Adam, defaultdict, dict]):
                state_dict = torch.load(f, map_location="cpu")
        try:
            load_state = state_dict['model'] if 'model' in state_dict.keys() else state_dict
            missing_keys, unexpected_keys = sam.load_state_dict(load_state, strict=False)
            if missing_keys:
                print(f"*******missing keys when loading checkpoint: {len(missing_keys)}")
                print(missing_keys[:8])
            if unexpected_keys:
                print(f"*******unexpected keys when loading checkpoint: {len(unexpected_keys)}")
                print(unexpected_keys[:8])
        except Exception:
            print('*******interpolate')
            new_state_dict = load_from(sam, state_dict, image_size, vit_patch_size)   
            missing_keys, unexpected_keys = sam.load_state_dict(new_state_dict, strict=False)
            if missing_keys:
                print(f"*******missing keys when loading interpolated checkpoint: {len(missing_keys)}")
                print(missing_keys[:8])
            if unexpected_keys:
                print(f"*******unexpected keys when loading interpolated checkpoint: {len(unexpected_keys)}")
                print(unexpected_keys[:8])
